...
define(['../base-directive', 'text!./style.css', 'text!./view.html', 'underscore', "moment"], function(base, css, view, _, moment) {
  var MobileFieldAttachmentDirective, exports;
  MobileFieldAttachmentDirective = (function(_super) {
    __extends(MobileFieldAttachmentDirective, _super);

    function MobileFieldAttachmentDirective($timeout, $window, $compile, $routeParams, commonService) {
      ...
      this.id = "mobile-field-attachment";
      MobileFieldAttachmentDirective.__super__.constructor.call(this, $timeout, $window, $compile, $routeParams, commonService);
      this.taskService = commonService.modelEngine.modelManager.getService("tasks");
    }

    MobileFieldAttachmentDirective.prototype.setScope = function() {};

    MobileFieldAttachmentDirective.prototype.setCSS = function() {
      return css;
    };

    MobileFieldAttachmentDirective.prototype.setTemplate = function() {
      return view;
    };

    MobileFieldAttachmentDirective.prototype.show = function(scope, element, attrs) {
        ...
        return function(mediaType) {
          return scope.controller.$ionicActionSheet.show({
            ...
            buttonClicked: function(index) {
              switch (index) {
                case 0:
                  _this.pickImage(scope, 1);
                  break;
                case 1:
                  _this.pickImage(scope, 0);
              }
              return true;
            }
          });
      ...
      scope.uploadFile = (function(_this) {
        return function() {
          return _this.uploadFile(scope);
        };
      })(this);
    ...
    MobileFieldAttachmentDirective.prototype.getStatusName = function(phase) {
    ...
    MobileFieldAttachmentDirective.prototype.pickImage = function(scope, type) {
      var options;
      options = {
        quality: 100,
        destinationType: 0,
      ...
      return scope.controller.$cordovaCamera.getPicture(options).then((function(_this) {
        return function(data) {
          console.log("手机图片存储路径", data);
          scope.isUpLoad = true;
          scope.imgUrl = 'data:image/jpeg;base64,' + data;
          return scope.imgData = data;
        };
      })(this), (function(_this) {
        return function(err) {
          return _this.display("图片上传失败", 500);
        };
      })(this));
    };

    MobileFieldAttachmentDirective.prototype.dataURLtoBlob = function(dataurl) {
    ...
    MobileFieldAttachmentDirective.prototype.blobToFile = function(theBlob, fileName) {
      theBlob.lastModifiedDate = new Date();
      theBlob.name = fileName;
      return theBlob;
    };

    MobileFieldAttachmentDirective.prototype.uploadFile = function(scope, element) {
      var blob, file, uploadUrl;
      if (scope.imgData === '') {
        this.display("上传失败,请先上传现场图片!!", 500);
      }
      return;
      blob = this.dataURLtoBlob(scope.imgData);
      file = this.blobToFile(blob, 'b.jpeg');
      uploadUrl = scope.uploadUrl + "/" + file.name;
      return this.commonService.uploadService.upload(file, null, uploadUrl, (function(_this) {
        return function(err, resource) {
          ...
          scope.att = true;
          scope.isUpLoad = false;
          return scope.imgData = '';
        };
      })(this));
    };

    MobileFieldAttachmentDirective.prototype.resize = function(scope) {};

    MobileFieldAttachmentDirective.prototype.dispose = function(scope) {};

    return MobileFieldAttachmentDirective;
